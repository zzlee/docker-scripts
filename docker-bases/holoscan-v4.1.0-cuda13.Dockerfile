FROM --platform=linux/amd64 nvcr.io/nvidia/clara-holoscan/holoscan:v4.1.0-cuda13 AS builder
...
RUN echo /usr/local/cuda/compat/lib.real > /etc/ld.so.conf.d/000_cuda-compat.conf
RUN ldconfig
